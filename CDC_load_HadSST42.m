% [HadSST42,lon,lat,yr] = CDC_load_HadSST42(en,P);
% P.do_random = 0; HadSST42 = CDC_load_HadSST42(en,P);
% P.do_random = 1; HadSST42 = CDC_load_HadSST42(en,P);
% P.do_random :: use ensemble members additionally perturbed for random errors
% 
% P.do_random = 1 - NOT YET DEVELOPED

function [HadSST4,lon,lat,yr] = CDC_load_HadSST42(en,P)

    data_version = '4.2.0.0';
    ...
    dir    = [CDC_other_temp_dir,'HadSST42/'];
    ...
    elseif en == -1
        ...
        file = [dir,'HadSST42_ensemble/HadSST.',data_version,'_ensemble_member_',num2str(en),'.nc'];
    end
    ...
    if en <= 0 || P.do_random == 0
        ...
        file_HadSST4 = [dir,'HadSST42_ensemble_perturbed/HadSST.',data_version,'_perturbed_ensemble_member_',num2str(en),'.mat'];
        
        if ~isfile(file_HadSST4)
        
            err1 = ncread([dir,'HadSST.',data_version,'_measurement_and_sampling_uncertainty.nc'],'tos_unc');
            ...
            for ct_yr = [1:Nt/12] + 1849
                dir_err  = [dir,'HadSST42_error_covariance/'];
                ...
                for ct_mon = 1:12
                    try
                        ...
                        l        = ~isnan(nanmean(tos_cov,1));
                        tos_cov_temp = tos_cov(l,l);
                        tos_cov_temp = (tos_cov_temp+tos_cov_temp')/2;
                        tos_cov_temp = tos_cov_temp + 1e-6 * eye(size(tos_cov_temp));
